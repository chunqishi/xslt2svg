...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="@rdf:resource" mode="lf-to-amr">
        ...
        <xsl:choose>
            <xsl:when test="contains($id-stack, concat(' ', $id, ' ')) or ../../preceding-sibling::rdf:Description/role:*[@rdf:resource=$frag]">
                <!-- already seen this ID before, just use the ID itself -->
                <xsl:call-template name="id">
                    <xsl:with-param name="id" select="$id" />
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
                <!-- this is the first use of this ID, look up its description -->
